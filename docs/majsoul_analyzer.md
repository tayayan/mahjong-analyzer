...
| `--extra <n>` | 期待値計算の探索範囲（既定 1、大きいほど正確で遅い） |
...
- **配牌直後など、テンパイから遠い局面** … 5 向聴以上では `--extra` の探索範囲を自動的に
  0 に落とします。期待値計算は現在の向聴数から `--extra` 分だけ悪い手も探索しますが、
  テンパイから遠いとこの探索が爆発し、1 局面に数分かかったうえでスタックを使い切って
  異常終了することがあるためです。この状況の打牌は僅差になりようがないので、精度より
  完走を優先しています。該当した局面数はサマリーに「探索を縮小」として表示されます。
  なお探索を縮小した局面では向聴数が悪化する打牌が候補に含まれないため、実際の打牌が
  それに当たると「候補に見つかりません」となり集計から外れます。
- **処理時間** … 1 局面あたり数十 ms〜数秒かかります。半荘 1 回で数十〜百局面になるため、
  急ぐときは `--max-shanten` と `--extra` で調整してください。
